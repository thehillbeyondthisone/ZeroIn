    ...
    public class MobTargeting
    {
        ...
        private IEnumerable<SimpleChar> GetPossibleTargets()
        {
            ...
            if (_config.PathingConfig.AttackPriorityOnly)
                return sharedQuery.Where(x => AutomatonRoamba.RoamPath.Rules.PriorityNames.Contains(x.Name));
            else 
                return sharedQuery.Where(x=> !AutomatonRoamba.RoamPath.Rules.IgnoredNames.Contains(x.Name));
        }
        ...
        private List<SimpleChar> FilterTargets(IEnumerable<SimpleChar> possibleTargets, Item tauntItem)
        {
            ...
            finalTargets = finalTargets
              ...
              .Where(x => AutomatonRoamba.RoamPath.SPath.Waypoints.Any(c => Vector3.Distance(c, x.Position) < _config.PathingConfig.WanderLimit))
              .ToList();
        ...
        private List<SimpleChar> OrderTargets(List<SimpleChar> finalTargets)
        {
            ...
            return finalTargets
                .OrderBy(x =>
                {
                    int index = AutomatonRoamba.RoamPath.Rules.PriorityNames.IndexOf(x.Name);
        ...
        {
            return target != null && _config.PathingConfig.UseTauntItem && target.IsInLineOfSight && Vector3.Distance(target.Position, DynelManager.LocalPlayer.Position) <= AutomatonRoamba.Config.PathingConfig.TauntRange;
        }
